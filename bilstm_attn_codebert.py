...
import json
import logging
from models import SimpleLSTM, SimpleLSTMConfig, SimpleBILSTM, SimpleBILSTMAttn, SimpleBILSTMAttnWithCodeBERT
...
logging.basicConfig(
    filename='bilstm_attn_codebert_log.txt',
    filemode='a',
    level=logging.INFO,
...
class SimpleCodeSmellDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Extract input_ids and remove the extra dimension
...
def train_epoch(model, dataloader, criterion, optimizer, device, pbar=None):
    ...
    for code, attention_mask, labels in dataloader:
        if pbar:
            pbar.update(1)
        ...
        optimizer.zero_grad()
        outputs = model(code, attention_mask = attention_mask)
...
def main():
    ...
    smelly_indices = [i for i, (_, _,label) in enumerate(dataset) if label != 4]
    ...
    logging.info(f"Balanced dataset size: {len(balanced_dataset)}")
    ...
    model = SimpleBILSTMAttnWithCodeBERT(model_config).to(device)
    logging.info(f"\nModel Architecture:\n{model}")
