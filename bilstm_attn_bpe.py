...
import json
import logging
from models import  SimpleLSTMConfig, SimpleBILSTMAttn
...
def main():
    ...
    logging.info(f"Number of Layers: {model_config.num_layers}")
    logging.info(f"Hidden dimension: {model_config.hidden_dim}")
    logging.info(f"Embedding dimension: {model_config.embedding_dim}")
    logging.info(f"Post LSTM Dropout: {model_config.dropout}")
    logging.info(f"Max seq len: {model_config.max_seq_len}")
